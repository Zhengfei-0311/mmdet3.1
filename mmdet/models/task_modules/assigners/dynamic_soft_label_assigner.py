...
class DynamicSoftLabelAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        valid_decoded_bbox_wh = (valid_decoded_bbox[:, 2:] - valid_decoded_bbox[:, :2])
        valid_prior = priors[valid_mask]
        strides = valid_prior[:, 2]
        distance_0 = (valid_prior[:, None, :2] - gt_center[None, :, :]
                    ).pow(2).sum(-1).sqrt() / strides[:, None]
        
        distance_1 = (valid_decoded_bbox_wh[:, None, :2] - gt_wh[None, :, :]
                      ).pow(2).sum(-1).sqrt() / strides[:, None]
        soft_center_prior = torch.pow(10, distance_0 +distance_1 - self.soft_center_radius)
